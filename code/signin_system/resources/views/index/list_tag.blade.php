...
<div class="title">操作</div>
	<a type="button" class="mui-btn mui-btn-primary mui-btn-block mui-btn-outlined" id="addmoon" style="padding:6px 0px" >新建读物</a>
<div class="title">已有读物</div>
<h5 class="mui-content-padded">右滑删除该读物</h5>
...
<script type="text/javascript">
	document.getElementById("addmoon").addEventListener('tap', function(e) {
		...
		var btnArray = ['取消', '确定'];
		mui.prompt('请输入你的读物','', '设置读物', btnArray, function(e) {
			if (e.index == 1) {
				var name = e.value;
				if (!name) {
					mui.toast('读物不得为空');
				} else {
					mui.ajax('/api/index/addtag',{
						data:{
							name:name
						},
...
<script>
	$.get('/api/index/listtagperson', function(data) {
		if(data.code == 200) {
	...
	$('.btn-del').click(function() {
		...
		$.get('/api/index/deltag?id='+id, function(data) {
			if(data.code == 204) {
				tag.hide();
			}
		});
